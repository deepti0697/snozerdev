...
class CreateFootballTeamVC: ViewController {
...
//        RunLoop.main.add(self.timer, forMode: RunLoop.Mode.common)
        ...
        HomeService.GetPlayers(_apiName: api, params: [:]) { (flag, response, msg) in
            if flag{
                if let info = response{
                    ...
                    self.arrPlayersCate1 = self.objTeamList.wicketkeapers
                    self.arrPlayersCate2 = self.objTeamList.batsmans
                    self.arrPlayersCate3 = self.objTeamList.allrounders
                    self.arrPlayersCate4 = self.objTeamList.bowlers
    ...
    func setUpGameSettings() {
        ...
        if let setting = matchSetting{
            ...
            self.lblPickupCount.text = "Pick \(self.Cat1MinimumPlayer) GOALKEEPER";
        }
    }
        ...
        case 0: ///--Filter By winning prize
            ...
            if isPlayer == false {
                ...
                if self.arrTeamPlayers.count > 0 {
                    let tempLeagueData : [PlayerData] = self.arrTeamPlayers
                    self.arrTeamPlayers.removeAll()
                    self.arrTeamPlayers = tempLeagueData.sorted(by: { ($0).playerName > ($1).playerName } )
                    self.tblPlayers.reloadData()
                }
        ...
        case 1: ///--Filter By team size
            ...
            if isPoints == false {
                ...
                if self.arrTeamPlayers.count > 0 {
                    ...
                    let tempLeagueData : [PlayerData] = self.arrTeamPlayers
                    self.arrTeamPlayers.removeAll()
                    self.arrTeamPlayers = tempLeagueData.sorted(by: { Double($0.totalPoints)! < Double($1.totalPoints)! } )
                    self.tblPlayers.reloadData()
                }
                UIView.animate(withDuration: 0.25, animations: {
    ...
    @IBAction func btnSelectPlayeTypeAction(_ sender: UIButton)
    {
        ...
        case 1: ///--for 2nd player category conditions
            ...
            self.lblPickupCount.text = "Pick \(self.Cat2MinimumPlayer) - \(self.Cat2MaximumPlayer) DEFENDER";
        ...
        case 2: ///--for 3rd player category conditions
            ...
            self.lblPickupCount.text = "Pick \(self.Cat3MinimumPlayer) - \(self.Cat3MaximumPlayer) MID-FIELDER";
        ...
        case 3: ///--for 4th player category conditions
            ...
            self.lblPickupCount.text = "Pick \(self.Cat4MinimumPlayer) - \(self.Cat4MaximumPlayer) FORWARD";
...
extension CreateFootballTeamVC : UITableViewDelegate, UITableViewDataSource , PlayerTeamSelectDelegate{
        ...
        if SinglePlayerData.isSelected == false && SelectedPlayer.count == totalPlayers {
            ...
            if self.SelectedPlayerType == 0 {
                if  SinglePlayerData.isSelected == false && Cat1SelectedPlayer == Cat1MaximumPlayer {
                    ...
                    self.view.showToast(message: "Max \(Cat1MaximumPlayer) GoalKeeper allowed")
                    return false
                } else {
                    return true
                }
                ...
                if  SinglePlayerData.isSelected == false && Cat2SelectedPlayer == Cat2MaximumPlayer {
                    ...
                    self.view.showToast(message: "Max \(Cat2MaximumPlayer) Defender allowed")
                    return false
                } else {
                    ...
                    if leftDeselectPlayer == ( max((Cat1MinimumPlayer - Cat1SelectedPlayer),0) + max((Cat4MinimumPlayer - Cat4SelectedPlayer), 0) + max((Cat3MinimumPlayer - Cat3SelectedPlayer),0)){
                        ...
                        if Cat1SelectedPlayer < Cat1MinimumPlayer {
                            self.view.showToast(message: "Pick at least \(Cat1MinimumPlayer) GoalKeeper");
                            return false
                        } else if Cat3SelectedPlayer < Cat3MinimumPlayer {
                            self.view.showToast(message: "Pick at least \(Cat3MinimumPlayer) Mid-Fielder");
                            return false
                        } else if Cat4SelectedPlayer < Cat4MinimumPlayer {
                            self.view.showToast(message: "Pick at least \(Cat4MinimumPlayer) Forward");
                            return false
                        } else {
                            return true
                        }
                ...
                if  SinglePlayerData.isSelected == false && Cat3SelectedPlayer == Cat3MaximumPlayer {
                    self.view.showToast(message: "Max \(Cat3MaximumPlayer) Mid-Fielder allowed");
                    return false
                } else {
                    ...
                    if leftDeselectPlayer == ( max((Cat1MinimumPlayer - Cat1SelectedPlayer),0) + max((Cat2MinimumPlayer - Cat2SelectedPlayer), 0) + max((Cat4MinimumPlayer - Cat4SelectedPlayer),0)){
                        ...
                        if Cat1SelectedPlayer < Cat1MinimumPlayer {
                            ...
                            self.view.showToast(message: "Pick at least \(Cat2MinimumPlayer) Defender");
                            return false
                        } else if Cat4SelectedPlayer < Cat4MinimumPlayer {
                            self.view.showToast(message: "Pick at least \(Cat4MinimumPlayer) Forward");
                            return false
                ...
                if  SinglePlayerData.isSelected == false && Cat4SelectedPlayer == Cat4MaximumPlayer {
                    self.view.showToast(message: "Max \(Cat4MaximumPlayer) Forward allowed");
                    return false
                } else {
