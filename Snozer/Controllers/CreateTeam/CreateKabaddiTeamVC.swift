...
class CreateKabaddiTeamVC: ViewController {
    
	@IBOutlet weak var layoutLblTimeY: NSLayoutConstraint!
	...
	self.layoutLblTimeY.constant = appdelegate.hasTopNotch ? 15 : 5;
...
// Do any additional setup after loading the view.
    ...
    func setUp() {
        ...
        self.vwPlayerPickupCount.setshadow(cornerR: 4, shadowC: .black, shadowO: 0.4, shadowR: 2, shadowOffset: CGSize(width: -1, height: -1));
        ...
        HomeService.GetPlayers(_apiName: api, params: [:]) { (flag, response, msg) in
            if flag{
                if let info = response{
                    ...
                    self.arrPlayersCate1 = self.objTeamList.bowlers
                    self.arrPlayersCate2 = self.objTeamList.batsmans
                    self.arrPlayersCate3 = self.objTeamList.allrounders
                    ...
                    if self.arrTeamPlayers.count > 0 {
                        let tempLeagueData : [PlayerData] = self.arrTeamPlayers
                        self.arrTeamPlayers.removeAll()
                        self.arrTeamPlayers = tempLeagueData.sorted(by: { Double($0.credits)! > Double($1.credits)! } )
                        self.isCredit = false
                    }
    ...
    func GetEditTeamPlayers() {
        if let info = self.editTeamData{
            ...
            self.arrTeamPlayers = self.arrPlayersCate1
            self.Cat1SelectedPlayer =  info.bowlers.count
    ...
    func setUpGameSettings() {
        ...
        if let setting = matchSetting{
            ...
            self.lblPickupCount.text = "Pick \(self.Cat1MinimumPlayer) - \(self.Cat1MaximumPlayer) DEFENDERS";
        }
    }
    ...
    @IBAction func btnSelectPlayeTypeAction(_ sender: UIButton)
    {
        ...
        case 1: ///--for 2nd player category conditions
            ...
            self.lblPickupCount.text = "Pick \(self.Cat2MinimumPlayer) - \(self.Cat2MaximumPlayer) RAIDERS";
        ...
        case 2: ///--for 3rd player category conditions
            ...
            self.lblPickupCount.text = "Pick \(self.Cat3MinimumPlayer) - \(self.Cat3MaximumPlayer) ALL-ROUNDERS";
...
extension CreateKabaddiTeamVC : UITableViewDelegate, UITableViewDataSource , PlayerTeamSelectDelegate {
    ...
    func selectPlayerAction(obj : PlayerData) {
        ...
        if isSuccess {
            ...
            //            arrTeamPlayers[indexPath.row]  = obj
            if self.SelectedPlayerType == 0 {
                //                self.arrPlayersCate1.removeAll()
                //                self.arrPlayersCate1 = self.arrTeamPlayers
                ...
                //                self.arrPlayersCate2[indexPath.row] = obj
                //                self.arrPlayersCate2.removeAll()
                //                self.arrPlayersCate2 = self.arrTeamPlayers
                ...
                //                self.arrPlayersCate3[indexPath.row] = obj
                //                self.arrPlayersCate3.removeAll()
                //                self.arrPlayersCate3 = self.arrTeamPlayers
            ...
            self.btnNext.backgroundColor = APPCOLOR.TextLightGrayColor;
            self.btnNext.setTitleColor(APPCOLOR.TextBlackColor, for: .normal);
            if self.arrSelectedTeamPlayers.count > 6{
                self.btnNext.backgroundColor = APPCOLOR.greenCustom;
                self.btnNext.setTitleColor(APPCOLOR.TextWhiteColor, for: .normal);
            }
            self.tblPlayers.reloadData()
    ...
    func CreateTeamLogic(_ SinglePlayerData : PlayerData , SelectedPlayer : [PlayerData]) -> Bool {
        ...
        if SinglePlayerData.isSelected == false && SelectedPlayer.count == totalPlayers {
            //            self.btnNext.backgroundColor = APPCOLOR.greenCustom;
            self.view.showToast(message: "Maximum \(totalPlayers) players allow")
            return false
            ...
            self.view.showToast(message: "Maximum \(MaxTeamPlayer) players are allow from 1 team")
            return false
        } else {
            ...
            if self.SelectedPlayerType == 0 {
                if  SinglePlayerData.isSelected == false && Cat1SelectedPlayer == Cat1MaximumPlayer {
                    ...
                    self.view.showToast(message: "Max \(Cat1MaximumPlayer) Defender allowed")
                    return false
                } else {
                    return true
                }
                ...
                if  SinglePlayerData.isSelected == false && Cat2SelectedPlayer == Cat2MaximumPlayer {
                    ...
                    self.view.showToast(message: "Max \(Cat2MaximumPlayer) Raider allowed")
                    return false
                } else {
                    
                    if leftDeselectPlayer == ( max((Cat1MinimumPlayer - Cat1SelectedPlayer),0) + max((Cat3MinimumPlayer - Cat3SelectedPlayer),0)){
                        
                        if Cat1SelectedPlayer < Cat1MinimumPlayer {
                            self.view.showToast(message: "Pick at least \(Cat1MinimumPlayer) Defender");
                            return false
                ...
                if  SinglePlayerData.isSelected == false && Cat3SelectedPlayer == Cat3MaximumPlayer {
                    self.view.showToast(message: "Max \(Cat3MaximumPlayer) All-Rounder allowed");
                    return false
                } else {
                    
                    if leftDeselectPlayer == ( max((Cat1MinimumPlayer - Cat1SelectedPlayer),0) + max((Cat2MinimumPlayer - Cat2SelectedPlayer), 0)){
                        
                        if Cat1SelectedPlayer < Cat1MinimumPlayer {
                            self.view.showToast(message: "Pick at least \(Cat1MinimumPlayer) Defender");
                            return false
                        } else if Cat2SelectedPlayer < Cat2MinimumPlayer {
                            self.view.showToast(message: "Pick at least \(Cat2MinimumPlayer) Raider");
                            return false
                        } else {
                            return true
                        }
